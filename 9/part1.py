...
def getParameter(command_pos):
    ...
    for x in range(2):
        if param[x] == 1:
            para[x] = computer[command_pos+x+1]
        elif param[x] == 2:
            para[x] = computer[relative_base+computer[command_pos+x+1]]
        else:
...
while True:
    ...
    if opcode == 99:
        ...
        print("Relative Base")
        relative_base += getFirstParameter(command_pos)
        step = 2
    else:
        print("Something has gone wrong; Opcode: ", opcode)
        break
    command_pos += step
