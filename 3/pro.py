...
runs = input.split('\n')
places = np.zeros(shape=(size*2,size*2), dtype=int)
current =  [size,size]
...
def walk(dir, nr):
    ...
    for i in range(int(nr)):
        ...
        places[current[0]][current[1]] = 1

def check(dir, nr):
    global current
    for i in range(int(nr)):
        current  = [current[0]+dir[0],current[1]+dir[1]]
        if places[current[0]][current[1]] > 0:
            places[current[0]][current[1]] += 1
...
for i in range(1,size):
    if i/2 > sums[-1]:
        break
    for j in range(1,i):
        current =[current[0]+sign, current[1]]
        if places[current[0],current[1]] > 1:
            dx = size - current[0] if current[0] < size else current[0]-size
            dy = size - current[1] if current[1] < size else current[1]-size
            sums.append(dx+dy)
            print(dx,'+',dy,'=',dx+dy)
    for j in range(1,i):
        current =[current[0], current[1]+sign]
        if places[current[0],current[1]] > 1:
            dx = size - current[0] if current[0] < size else current[0]-size
            dy = size - current[1] if current[1] < size else current[1]-size
            sums.append(dx+dy)
