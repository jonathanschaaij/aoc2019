...
def walk(dir, nr,run):
    global current, step, sums
    for i in range(int(nr)):
        current  = [current[0]+dir[0],current[1]+dir[1]]
        step += 1
        if places[current[0]][current[1]][run] == 0:
            places[current[0]][current[1]][run] = np.int32(step)
        else:
            continue

        if run==1:
            if places[current[0]][current[1]][0] != 0:
                d0 = places[current[0]][current[1]][0]
                d1 = places[current[0]][current[1]][1]
                sums.append(d0+d1)
                print(d0,d1,d0+d1)

sums = []
size = np.int32(9000)
places = np.zeros(shape=(size*2,size*2,2), dtype=np.int32)
for run in range(2):
    step = 0
    current =  [size,size]
    parts = runs[run].split(',')
    for part in parts:
        if part[0] == 'R':
            walk([1,0],part[1:],run)
        elif part[0] == 'L':
